...
- they allow to test aesthetics, which is difficult with other assertion methods
...
2. Run Playwright with the environment variable `PW_TEST_CONNECT_WS_ENDPOINT` set:
...
- Optional: add a `PWDEBUG` escape hatch so `playwright test --debug` runs with local browsers:
  ...
  ```typescript
  // set-env.ts
  const useDocker = process.env['PWDEBUG'] !== '1';
