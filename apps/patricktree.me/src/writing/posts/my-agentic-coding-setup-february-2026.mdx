...
- **Claude Code:** Heavily collapses tool calls and thinking traces. Has slow path autocompletion. I don't need the checkpoints features (just using git). Flickers.
...
- **"Run full validation when you think you are finished."** This one instruction makes even gpt-5.2-codex actually run tests when it's done 😉.
...
- Not using voice dictation. I tried it but it gets into my way when I need to reference files/directories or adding images. Not feeling the value of it. Some people say the Pre-November-2025 models needed more guidance, more human input, so maybe the tradeoff was worth it back than.
