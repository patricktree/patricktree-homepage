...
lastUpdatedAtISO: "2026-08-31T12:00:00.000Z"
...
<VersionTabs versions={["TS 5.9 Node.js", "TS 6.0/7.0 Node.js", "TS 6.0/7.0 Browser"]} defaultVersion="TS 6.0/7.0 Node.js">
...
<Version values={["TS 6.0/7.0 Node.js"]}>
...
<Version values={["TS 6.0/7.0 Browser"]}>
...
<Version values={["TS 5.9 Node.js", "TS 6.0/7.0 Node.js"]}>
...
- ESM-exclusive features like `import ... with { }` statements and `import.meta` will be forbidden in CJS source files.
...
In TypeScript 6.0, the default `rootDir` will always be the directory containing the `tsconfig.json` file (except when using `tsc` from the command line without a `tsconfig.json` file, then `rootDir` will be inferred, more infos <FancyAnchor href="https://devblogs.microsoft.com/typescript/announcing-typescript-6-0/#rootdir-now-defaults-to-.">here</FancyAnchor>).
...
{/* prettier-ignore */}<FancyAnchor href="https://www.typescriptlang.org/tsconfig#noEmit">#noEmit docs</FancyAnchor>
...
{/* prettier-ignore */}<FancyAnchor href="https://www.typescriptlang.org/tsconfig#erasableSyntaxOnly">#erasableSyntaxOnly docs</FancyAnchor>

`erasableSyntaxOnly` rejects TypeScript features that need code generation, like `enum`, parameter properties, runtime `namespace`s, `import = require(...)`, and `export =`.
...
That is why the <FancyAnchor href="https://nodejs.org/api/typescript.html">Node.js TypeScript docs</FancyAnchor> recommend this option.
...
I bumped it to `ES2022` because browsers have supported it for a long time; Node.js 22, the oldest maintained Node.js release, supports it; and it enables the use of the `cause` option on `Error` objects.
...
<Version values={['TS 6.0/7.0 Node.js', 'TS 6.0/7.0 Browser']}>
