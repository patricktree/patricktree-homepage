...
<VersionTabs versions={["TS 5.9", "TS 6.0 Node.js", "TS 6.0 Browser"]} defaultVersion="TS 6.0 Node.js">

<Version values={["TS 5.9"]}>
...
1. **Supported Module Features:** Whether to report the usage of certain module features as errors or not. E.g. `import.meta.resolve`, `import ... with { }` statements, or `require(esm)` from CJS source files.
2. **Module Emit:** Whether to transform ESM `import` statements, dynamic `import()`s and ESM `export` statements in source files to CommonJS `require` function calls and `module.exports` assignments in output files.

<Version values={["TS 6.0 Node.js"]}>

`"Node20"` models the behavior of Node.js 20+.

Each TypeScript source file is treated as ESM or CJS based on `type` of the "nearest" `package.json` file (that is, in the same directory or in any ancestor directory of the TypeScript source file) - matching what Node.js actually does at runtime.
...
This is correct, Node.js would throw an error during runtime in these cases.  
...
If you want to use any module features supported by a newer `"NodeXX"` module value, upgrade to it but also make sure that the code only runs on that Node.js version or above.
...
`moduleResolution`: `"Node16"` resolves `package.json#exports` and `#imports`, as Node.js and tools like Vite do.  
It also forbids extensionless and directory imports in ESM files, matching Node.js behavior.
...
**`moduleResolution`: `"Bundler"`** resolves `package.json#exports` and `#imports`.  
It allows extensionless and directory imports, which is more relaxed than Node.js.
...
<Version values={["TS 5.9", "TS 6.0 Node.js"]}>
